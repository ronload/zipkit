...
function toNeutral(color: string): string {
  ...
  if (c.a < 1) return `rgba(${g}, ${g}, ${g}, ${c.a})`;
...
function neutralizeMapColors(map: MapLibreGL.Map, theme: Theme) {
  ...
  useEffect(() => {
    ...
    const styleDataHandler = () => {
      ...
      styleTimeoutRef.current = setTimeout(() => {
        // Neutralize Carto basemap colors when using default styles
        if (
          currentStyleRef.current === defaultStyles.dark ||
          currentStyleRef.current === defaultStyles.light
        ) {
          neutralizeMapColors(map, resolvedThemeRef.current);
        }
